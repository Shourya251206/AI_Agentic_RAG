...
Your role is to intelligently select and coordinate specialized agents to address complex product research and development queries, as well as general or conversational queries. 
...
that includes the selected agents and a clear, concise justification for your choices. If the query is generic, conversational, or a greeting, select BingDataRetrievalAgent by default and explain why.
"""
...
def generate_user_prompt(user_query: str) -> str:
    ...
    - If the query is generic, conversational, or a greeting (e.g., "hello", "hi", "how are you?", "what's up?"), select BingDataRetrievalAgent by default and justify that it is best suited for general queries or small talk.
    ...
    1. 📊 **Structured Data** → Use **FabricDataRetrievalAgent**
       - Used when the query involves:
         ...
         - **Sales data, financial metrics, revenue, quarterly results, or anything mentioning 'dataset', 'Q4', 'quarter', 'financial', 'revenue', 'sales', or 'Fabric datasets'**
       - Trigger words: `accuracy`, `metrics`, `performance`, `range`, `threshold`, `experiments`, `sales`, `dataset`, `Q4`, `quarter`, `financial`, `revenue`, `Fabric datasets`
    ...
    3. 🌐 **Web-based Information** → Use **BingDataRetrievalAgent**
       - Used when:
         ...
         - Or when the query is generic, conversational, or a greeting
       - Trigger words: `latest`, `news`, `on the web`, `from internet`, `recent`, `external`, `Google it`, or general greetings
    ...
    - If none of the agents match, but the query is generic, conversational, or a greeting, select BingDataRetrievalAgent and justify that it is best for general queries or small talk.
    - If none of the agents match and the query is not generic, return an empty agent list with a clear explanation.
    ...
    If no agent fits and the query is not generic:
    ...
    If the query is generic, conversational, or a greeting:

    ```json
    {{
      "agents_needed": ["BingDataRetrievalAgent"],
      "justification": "The query is generic or conversational, so BingDataRetrievalAgent is best suited to provide a helpful response."
    }}
    ```
    ...
    **User Query:**  
    "Provide recent sales data from Fabric datasets for Q4 2023."

    **Result:**

    ```json
    {{
      "agents_needed": ["FabricDataRetrievalAgent"],
      "justification": "The user requested recent sales data from Fabric datasets for Q4 2023, which is structured financial data."
    }}
    ```
    ...
    **Result:**
    ...
    ### 💬 Example 6 (Generic/Conversational):
    **User Query:**  
    "hello buddy"
